...
export const MapScreen = () => {
    ...
    const getGeoCode = async () => {
      ...
      if(selecQueryType === 'start'){
        try{
          const res:any = await axios.get(`${url}?query=${Querydata.startQuery}&display=5&sort=random`, {headers})
      ...
      if(selecQueryType === 'end'){
        try{
          const res:any = await axios.get(`${url}?query=${Querydata.endQuery}&display=5&sort=random`, {headers})
          setSearchData(res.data.items)
...
<View>
    ...
    <NaverMapView style={{width: '100%', height: '60%'}}
        ...
        //  onCameraChange={e => console.log('onCameraChange', JSON.stringify(e))}
          ...
          <Marker
          key={index}
          coordinate={{latitude : Number(item.mapy) / Math.pow(10,7), 
